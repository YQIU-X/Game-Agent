#!/usr/bin/env python
# -*- coding: utf-8 -*-

"""
高效MP4转GIF转换脚本
使用更高效的方法处理大文件
"""

import imageio
import os
from PIL import Image

def convert_mp4_to_gif_efficient(mp4_path, gif_path, fps=8, max_width=800):
    """
    高效将MP4文件转换为GIF
    
    Args:
        mp4_path: MP4文件路径
        gif_path: 输出GIF文件路径
        fps: GIF帧率
        max_width: 最大宽度
    """
    print(f"开始高效转换: {mp4_path} -> {gif_path}")
    
    # 检查文件是否存在
    if not os.path.exists(mp4_path):
        print(f"错误: 文件不存在 {mp4_path}")
        return False
    
    try:
        # 读取MP4文件
        reader = imageio.get_reader(mp4_path)
        fps_original = reader.get_meta_data().get('fps', 30)
        print(f"原始视频帧率: {fps_original} FPS")
        print(f"目标GIF帧率: {fps} FPS")
        
        # 计算跳帧间隔
        skip_frames = max(1, int(fps_original / fps))
        print(f"跳帧间隔: {skip_frames}")
        
        frames = []
        frame_count = 0
        max_frames = 500  # 限制最大帧数
        
        for i, frame in enumerate(reader):
            if i % skip_frames == 0 and frame_count < max_frames:
                # 调整尺寸以控制文件大小
                img = Image.fromarray(frame)
                if img.width > max_width:
                    ratio = max_width / img.width
                    new_height = int(img.height * ratio)
                    img = img.resize((max_width, new_height), Image.Resampling.LANCZOS)
                
                frames.append(img)
                frame_count += 1
                
                if frame_count % 50 == 0:
                    print(f"已处理 {frame_count} 帧...")
        
        print(f"总共处理了 {frame_count} 帧")
        
        # 保存为GIF
        if frames:
            frames[0].save(
                gif_path,
                save_all=True,
                append_images=frames[1:],
                duration=int(1000 / fps),  # 毫秒
                loop=0,  # 无限循环
                optimize=True,  # 启用优化
                quality=85  # 高质量
            )
            
            # 显示文件大小
            file_size = os.path.getsize(gif_path) / (1024 * 1024)  # MB
            print(f"GIF文件大小: {file_size:.2f} MB")
            print(f"转换完成: {gif_path}")
            return True
        else:
            print("错误: 没有提取到任何帧")
            return False
            
    except Exception as e:
        print(f"转换失败: {e}")
        return False

def main():
    """主函数"""
    # 转换developer.mp4
    mp4_path = "static/developer.mp4"
    gif_path = "static/developer.gif"
    
    print("开始转换developer.mp4...")
    
    if convert_mp4_to_gif_efficient(mp4_path, gif_path, fps=8, max_width=800):
        print("✅ 转换成功完成!")
    else:
        print("❌ 转换失败!")

if __name__ == "__main__":
    main()
